...
from platform_admin.utilities import tenant_schema_from_request
...
def data_handling(
    ...
    if db_type == "MSSQL":
        ...
        postgres_push(table, DB_table_name, schema)
        redis_instance.delete(db_connection_name + DB_table_name)
...
def read_data_func(
    ...
    if db_type == "MSSQL":
        ...
        if columnlist != ["*"]:
            ...
            cursor1.execute(
                f"SELECT COLUMN_NAME FROM information_schema.columns WHERE table_name = '{sql_table_name}' AND table_schema = 'public'"
            )
        ...
        with open(f"{PLATFORM_FILE_PATH}user_databases.json") as json_file:
            ...
            if db_data["db_type"] == "MSSQL":
                ...
                try:
                    ...
                    logging.warning(f"Following exception occured - {e}")
            json_file.close()
    return [engine_temp, turbodbc_con]
...
def postgres_push(data_table, db_table_name, schema):
    sql_engine = psycopg2.connect(
        dbname=central_database_config["dbname"],
        user=central_database_config["user"],
        password=central_database_config["password"],
