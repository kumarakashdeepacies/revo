...
mxConnectionHandler.prototype.connectImage = new mxImage('images/connector.gif', 16, 16);
		...
		function mxIconSet(state)
		{
			...
			img.style.height = '16px';
			img.style.left = (state.x + state.width+5) + 'px';
			console.log("Copy-"+(state.y+25) + 'px')
			...
			img.style.height = '16px';
			img.style.left = (state.x + state.width+5) + 'px';
			console.log("Configure-"+(state.y + 50) + 'px')
			...
			mxEvent.addListener(img, 'click',
				mxUtils.bind(this, function(evt)
				{
					...
					if(shape=="rounded=1"){
						...
						//hreF="http://127.0.0.1:8000/users/CountryMaster"
					}
					else if(shape=="rhombus"){
						$('#alertModal').modal('show')
						//hreF="http://127.0.0.1:8000/users/CurrencyMaster"
					}
					else if(shape=="ellipse"){
						$('#chartModal').modal('show')
						//hreF="http://127.0.0.1:8000/users/Entity"
					}
					else{
...
EditorUi.prototype.refresh = function (sizeDidChange) {
	...
	if (this.toolbar != null) {
		this.toolbarContainer.style.top = '70px';
	...
	this.sidebarContainer.style.top = '100px';
	this.sidebarContainer.style.width = effHsplitPosition + 'px';
	this.formatContainer.style.top = '100px';
	...
	this.diagramContainer.style.top = '100px';
...
EditorUi.prototype.createDivs = function () {
	...
	this.menubarContainer.style.top = '41px';
		...
		try {
			if (Editor.useLocalStorage) {
				...
				if (xml.length < MAX_REQUEST_SIZE) {
					...
					new mxXmlRequest(SAVE_URL, 'filename=' + encodeURIComponent(name) +
						'&xml=' + xml).simulate(document, '_self');
				}
				else {
